...
def read_bounding_box_data(json_path):
    ...
    for item in annotations:
        image_id = item['image_id'].replace("jldp:", "")
        original_img_info = images_info.get(item['image_id'], {})
...
class TripletDataset(Dataset):
    ...
    def load_transform_image(self, image_path):
        try:
            ...
            with Image.open(image_path) as img:
                ...
                if bbox:  # Only apply cropping if bbox is valid
    ...
    def save_transformed_image(self, img, image_id):
        ...
        #print(f"Saving to path: {save_path}")  # Debugging: Check the save path
        img.save(save_path)


    
    def crop_resize_image(self, image, bbox):
        # bbox is expected to be [x_min_scaled, y_min_scaled, width_scaled, height_scaled]
        x_min, y_min, width, height = bbox
        x_max, y_max = x_min + width, y_min + height
        cropped_image = image.crop((x_min, y_min, x_max, y_max))
        cropped_image = cropped_image.resize((224, 224))  # Resize to the desired size
...
class SimCLRDataset(Dataset):
    ...
    def load_transform_image(self, image_path):
        try:
            ...
            with Image.open(image_path) as img:
                ...
                if self.save_sample_images and self.image_counter % 100 == 0: #move inside bbox if function if you only want to test for cropped photos
                    self.save_transformed_image(img, image_id)
                self.image_counter += 1
            ...
            print(f"Error loading image {image_path}: {e}")
            return torch.zeros([3, 224, 224])  # Return a tensor of zeros
    ...
    def save_transformed_image(self, img, image_id):
        save_path = os.path.join(self.save_dir, f"{image_id}_transformedsim.jpg")
        img.save(save_path)
...
def main():
    ...
    train_and_validate(encoder, train_loader, val_loader, epochs=10, optimizer=optimizer, criterion=criterion, device=device, run_folder=run_folder)
